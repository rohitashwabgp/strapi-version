...
const { isRelationalAttribute, getVisibleAttributes, isMediaAttribute, isTypedAttribute } = require("@strapi/utils").contentTypes;

/**
 * 
 * @param {*} modelOrAttribute 
 * @returns 
 */
...
const isVersionedAttribute = (model, attributeName) => {
  ...
  return (hasVersionedOption(attribute) || (isRelationalAttribute(attribute) && !isMediaAttribute(attribute)) || isTypedAttribute(attribute, "uid")
  );
};
...
const removeIdsMut = (model, entry) => {
  ...
  _.forEach(model.attributes, (attr, attrName) => {
    ...
    if (attr.type === "dynamiczone" && isArray(value)) {
      ...
      if (value) {
        entry[attrName] = isArray(value) ? value.map((r) => get(r, "id", r)) : get(value, "id", value);
      }
    }
  });
...
const getVersionedAttributes = (model) => {
  return getVisibleAttributes(model).filter((attributeName) => isVersionedAttribute(model, attributeName));
};
